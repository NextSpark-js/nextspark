...
> **NOTE**: These examples show the rendered OUTPUT in the DOM (what Playwright sees), NOT the source code. In the source code, developers must use the `sel()` function: `data-cy={sel('auth.login.submit')}`. The rendered result will be `data-cy="login-submit"`.
...
### Step 3: If Errors Found → Call Developer
...
### Step 4: Document Results for qa-automation

**IMPORTANT: qa-automation (Phase 15) inherits context from qa-manual.**

The qa-automation agent reads context.md to:
- Identify errors found and corrected
- Prioritize tests in problematic areas
- Avoid duplicating debugging effort

**MANDATORY structure for context inheritance:**

```typescript
interface QaManualContext {
  errorsFound: Array<{
    type: 'api_error' | 'ui_error' | 'navigation_error' | 'console_error' | 'server_error'
    location: string         // URL or affected component
    description: string      // Clear error description
    wasFixed: boolean        // Whether it was fixed in retry
    fixedBy?: 'backend-developer' | 'frontend-developer'
  }>
  flowsValidated: string[]   // URLs/flows navigated successfully
  problematicAreas: string[] // Areas that had problems (even if corrected)
  retryCount: number         // Number of retries executed
}
```
...
**Dashboard (superadmin@cypress.com):**
- [x] /dashboard/products - No errors
- [x] /dashboard/products/new - No errors
- [x] /dashboard/orders - No errors
- [x] /dashboard/customers - No errors

**Frontend:**
- [x] /products - No errors
- [x] /about - No errors
...
**Next Step:** Proceed with qa-automation (Phase 15)
```
...
**Attempt 1:** ❌ FAILED
- Error at /dashboard/products: Console error "Cannot read property 'map' of undefined"
- Action: Called frontend-developer for fix

**Attempt 2:** ❌ FAILED
- Error at /api/v1/products: 500 Internal Server Error
- Action: Called backend-developer for fix
...
**Status:** 🚫 GATE FAILED - BLOCKED (3/3 retries exhausted)

**Persistent Errors:**
1. /dashboard/products: React hydration error persists
2. /api/v1/orders: 500 error not resolved

**Screenshots:** qa-manual-error-*.png

**Action Required:** Manual review by senior developer.

**Documented in:** pendings.md for v2

**BLOCKED:** Do not proceed to qa-automation
```
