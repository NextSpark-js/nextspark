---
description: "[Step 3] Execute complete 17-phase workflow v4.3 with gates - ORCHESTRATOR ONLY (delegates ALL work to specialized agents)"
---

# Task Execution - 17-Phase Development Workflow v4.3
...
BLOQUE 2: DATABASE [CONDITIONAL]
+- Phase 3: db-developer (migrations + sample data)
+- Phase 4: db-validator [GATE]

BLOQUE 3: BACKEND (TDD)
+- Phase 5: backend-developer (tests first + implementation)
+- Phase 5.5: unit-test-writer (Jest unit tests)
+- Phase 6: backend-validator [GATE]
+- Phase 7: api-tester [GATE]

BLOQUE 4: BLOCKS [CONDITIONAL]
+- Phase 8: block-developer (if requires blocks)

BLOQUE 5: FRONTEND
+- Phase 9: selectors-translator (define data-cy + i18n contracts)
...
+- Phase 16: documentation-writer [OPTIONAL]
...
3. Login as superadmin@cypress.com
...
Analyze the session and context.md to report what was developed:

```markdown
## Workflow v4.3 Complete!
...
#### 📦 Components
- **Created:** [list new components created]
- **Updated:** [list existing components modified]
- **Location:** `contents/themes/{theme}/components/...`

#### 🎯 Selectors & Translations
- **data-cy selectors:** [count] defined in tests.md
- **Translation keys:** [count] added (EN + ES)
- **Status:** ✅ All documented / ⚠️ Missing [list]

#### 🧪 Cypress Tests
- **API Tests:** [count] tests in `e2e/api/{feature}/`
- **UAT Tests:** [count] tests in `e2e/uat/{feature}/`
- **BDD Docs:** [count] .bdd.md files generated
- **Pass Rate:** [X]%

#### 🔌 API Endpoints
- **Created:** [list new endpoints]
- **Updated:** [list modified endpoints]
- **Auth:** Dual auth (session + API key) verified

#### 🗄️ Database
- **Migrations:** [list migration files]
- **Sample Data:** [count] records per entity
- **Test Users:** Configured in devKeyring
...
| Gate | Phase | Status |
|------|-------|--------|
| db-validator | 4 | ✅ PASSED / ⏭️ SKIPPED |
| backend-validator | 6 | ✅ PASSED |
| api-tester | 7 | ✅ PASSED |
| frontend-validator | 11 | ✅ PASSED |
| functional-validator | 12 | ✅ PASSED |
| qa-manual | 13 | ✅ PASSED (attempt [N]/3) |
| qa-automation | 14 | ✅ PASSED ([X]% pass rate) |

---

### 🚀 Optional Next Steps

The core workflow is complete. Choose what to do next:
...
## ERROR HANDLING REFERENCE (v4.3)

| Gate | Phase | On Failure | Action |
|------|-------|------------|--------|
| db-validator | 4 | BLOCK | Launch db-developer to fix, retry gate |
| backend-validator | 6 | BLOCK | Launch backend-developer to fix, retry gate |
| api-tester | 7 | BLOCK | Launch backend-developer to fix, retry gate |
| frontend-validator | 11 | Agent fixes directly | Continue |
| functional-validator | 12 | BLOCK if major | Launch frontend-developer to fix |
| qa-manual | 13 | RETRY (max 3) | Launch appropriate developer, retry |
| qa-automation | 14 | BLOCK | Launch appropriate developer, retry |
