...
if (sessionPath) {
  await Edit({
    ...
    old_string: '[frontend-validator documents components and required data-cy selectors]',
    new_string: `### Documented data-cy Selectors

| Component | Selector | Description |
|-----------|----------|-------------|
| Page Title | \`${entityName}-page-title\` | Page title |
| Container | \`${entityName}-container\` | Main container |
| List | \`${entityName}-list\` | Items list |
| Item | \`${entityName}-item\` | Individual item |
| Card | \`${entityName}-card-{id}\` | Card with dynamic ID |
| Create Button | \`${entityName}-create-button\` | Create new button |
| Edit Button | \`${entityName}-edit-{id}\` | Edit button |
| Delete Button | \`${entityName}-delete-{id}\` | Delete button |
| Form | \`${entityName}-form\` | Form |
| Name Input | \`${entityName}-name-input\` | Name field |
| Submit Button | \`${entityName}-submit-button\` | Submit button |
| Dialog | \`${entityName}-dialog\` | Confirmation modal |
| Search Input | \`${entityName}-search-input\` | Search field |
| Empty State | \`${entityName}-empty-state\` | Empty state |
| Loading State | \`${entityName}-loading\` | Loading state |`
  })
}
```
...
\`\`\`gherkin:es
Scenario: User can access the ${entityName} list
