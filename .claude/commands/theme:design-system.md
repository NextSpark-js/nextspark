...
Step 2: Check Existing globals.css → ASK USER
Step 3: Route to Appropriate Path
  Path 1: Process CSS File
  Path 2: Analyze Mock → Generate CSS (INFORM about dark mode)
...
```markdown
⚠️ Missing required CSS variables:
{missingVars.join('\n')}
...
## Path 2: Analyze Mock → Generate CSS
...
```markdown
ℹ️ **Dark mode generado automáticamente**

El mock no incluía dark mode explícito. Se generó automáticamente
invirtiendo los valores de lightness (L) en OKLCH.

**Ejemplo de inversión:**
- Light: `oklch(0.95 0.02 250)` → Dark: `oklch(0.05 0.02 250)`
- Light: `oklch(0.15 0 0)` → Dark: `oklch(0.85 0 0)`
...
if [ -f "themes/{THEME}/styles/globals.css" ]; then
  ...
  echo "✅ Backup created: globals.css.bak"
fi
```
...
```markdown
## ✅ Written: themes/{THEME}/styles/globals.css
