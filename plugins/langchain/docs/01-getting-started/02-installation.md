...
- Node.js 22.14.0 or later with pnpm
...
cd plugins/langchain/migrations
...
cp plugins/langchain/presets/lib/langchain.config.ts.preset \
   lib/langchain/langchain.config.ts

# Copy API endpoint preset
mkdir -p api/ai/chat
cp plugins/langchain/presets/api/chat/route.ts.preset \
   api/ai/chat/route.ts

# Copy tools preset
mkdir -p lib/langchain/tools
cp plugins/langchain/presets/lib/tools/entity-tools.ts.preset \
   lib/langchain/tools/example.ts

# Copy prompt preset
mkdir -p lib/langchain/agents
cp plugins/langchain/presets/agents/entity-assistant.md.preset \
   lib/langchain/agents/assistant.md
```
...
```typescript
import { isProviderAvailable, getAvailableProviders } from '@nextsparkjs/plugin-langchain/plugin.config'
...
```typescript
import { memoryStore } from '@nextsparkjs/plugin-langchain/lib/memory-store'
...
```bash
# Run the migration
psql $DATABASE_URL -f plugins/langchain/migrations/001_langchain_memory.sql
```
