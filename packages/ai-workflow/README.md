...
> **Legacy opt-in pack.** New projects use the small versioned guidance pointers generated by the CLI instead of installing this package. To read a bundled offline guide that matches the project CLI, run `pnpm nextspark skills list` and `pnpm nextspark skills get <name>`. This workflow remains available only when explicitly selected with `pnpm nextspark setup:ai`.
