...
test('the missing line is appended to an existing .gitignore, once', async () => {
  ...
  try {
    ...
    assert.deepEqual(ensureGeneratedPathsIgnored(root, projectFiles(root)), ['src/app/(templates)/', 'app.backup.v*/'])
...
test('tracked files under src/app/(templates) are listed, and a project outside git has none', async () => {
  const { root, cleanup } = await project()
  try {
    await mkdir(join(root, 'src/app/(templates)/dashboard'), { recursive: true })
    await writeFile(join(root, 'src/app/(templates)/dashboard/layout.tsx'), 'export default function L() { return null }\n')

    assert.deepEqual(trackedTemplatesFiles(root), [])

    execFileSync('git', ['init', '-q'], { cwd: root })
    execFileSync('git', ['add', 'src/app'], { cwd: root })

    assert.deepEqual(trackedTemplatesFiles(root), ['src/app/(templates)/dashboard/layout.tsx'])
  } finally {
    await cleanup()
  }
})
...
test('sync:app backups end up ignored in a .gitignore that does not ignore .nextspark', async () => {
  ...
  try {
    ...
    assert.equal(gitIgnores(root, 'src/app/(templates)/dashboard/layout.tsx'), true)
...
test('src/app/(templates) counts as ignored only when every file of the tree is, not one of them', async () => {
  const cases: { name: string; gitignore: string; nested?: [string, string] }[] = [
    { name: 'a rule under **', gitignore: '**/dashboard/layout.tsx\n' },
    { name: 'a .gitignore inside the tree', gitignore: '', nested: ['src/app/(templates)/dashboard/.gitignore', 'layout.tsx\n'] },
    {
      name: 'rules that name the shapes but not a file the run writes',
      gitignore: 'src/app/(templates)/middleware.ts\nsrc/app/(templates)/dashboard/\nsrc/app/(templates)/(public)/\n',
    },
  ]
  const written = ['src/app/(templates)/dashboard/layout.tsx', 'src/app/(templates)/(auth)/login/page.tsx']
  ...
  for (const { name, gitignore, nested } of cases) {
    ...
    try {
      ...
      assert.deepEqual(ensureGeneratedPathsIgnored(root, projectFiles(root)), ['src/app/(templates)/'], name)
      for (const file of [...written, 'src/app/(templates)/(public)/[locale]/page.tsx']) {
...
test('src/app/(templates) counts as ignored only as a directory git leaves out whole, whatever rules name the files under it', async () => {
  const cases: { name: string; rules: string; missing: boolean }[] = [
    ...
    { name: 'a file of each shape', rules: 'src/app/(templates)/middleware.ts\nsrc/app/(templates)/dashboard/layout.tsx\nsrc/app/(templates)/(public)/page.tsx\n', missing: true },
    { name: 'the directory, at any depth', rules: '**/(templates)/\n', missing: false },
    { name: 'everything in app', rules: 'src/app/*\n', missing: false },
  ]
  const files = ['src/app/(templates)/custom/new.tsx', 'src/app/(templates)/deep/with spaces/área/"quoted" (1)/page.tsx', 'src/app/(templates)/new\nline/page.tsx']
  ...
  for (const { name, rules, missing } of cases) {
    ...
    try {
      ...
      if (JSON.stringify(got) !== JSON.stringify(missing ? ['src/app/(templates)/'] : [])) wrong.push(`${name}: added ${JSON.stringify(got)}`)
      // What the build writes, with a .gitignore of its own in the tree that takes all of it back
      for (const file of [...files, 'src/app/(templates)/.gitignore']) {
...
test('the paths on disk are every file under the places sync:app and the registry build write, and nothing else', async () => {
  ...
  try {
    const files = [
      'src/app/(templates)/deep/with spaces/área/"quoted" (1).tsx',
      '.nextspark/registries/entities.ts',
      '.nextspark/backups/2026-09-16T00-00-00-000Z-q1w2e3/src/app/(templates)/dashboard/layout.tsx',
      '.nextspark/sync-state.json',
    ...
    const others = ['src/app/page.tsx', '.nextspark/entities.ts', 'app.backup.vnotes.txt', 'app.backup/layout.tsx']
...
test('a .gitignore that is a symlink is not written through', async () => {
  ...
  try {
    ...
    assert.deepEqual(planGitignore(root), {
      add: [],
      leftForGit: ['src/app/(templates)/', '.nextspark/registries/', '.nextspark/backups/', '.nextspark/sync-state.json', 'app.backup.v*/'].map((entry) => ({
        entry,
...
test('a line the .gitignore has below its last negation is not added again, and one above a negation is', async () => {
  ...
  try {
    ...
    const written = ['src/app/(templates)/dashboard/layout.tsx']
    await mkdir(join(root, 'src', 'app'), { recursive: true })
    await writeFile(join(root, 'src/app/.gitignore'), '!(templates)/\n')

    await writeFile(join(root, '.gitignore'), `${rest}src/app/(templates)/\n`)
    assert.equal(gitIgnores(root, written[0]), false, 'src/app/.gitignore takes the tree back')
    assert.deepEqual(missingGitignoreEntries(root, written), [])
    assert.deepEqual(ensureGeneratedPathsIgnored(root, projectFiles(root)), [])

    await writeFile(join(root, 'src/app/.gitignore'), '')
    await writeFile(join(root, '.gitignore'), `${rest}src/app/(templates)/\n!src/app/(templates)/\n`)
    assert.deepEqual(ensureGeneratedPathsIgnored(root, projectFiles(root)), ['src/app/(templates)/'])
    assert.equal(gitIgnores(root, written[0]), true)
  } finally {
    await cleanup()
  }
...
test('a line counts as the entry only as git reads it: a CRLF or trailing spaces do not change it, a leading space or a trailing tab does', async () => {
  ...
  try {
    ...
    await mkdir(join(same.root, 'src/app'), { recursive: true })
    await writeFile(join(same.root, 'src/app/.gitignore'), '!(templates)/\n')
  ...
  for (const line of [' src/app/(templates)/', 'src/app/(templates)/\t', 'src/app/(templates)/\u00a0']) {
    const { root, cleanup } = await project()
    try {
      execFileSync('git', ['init', '-q'], { cwd: root })
      ...
      assert.deepEqual(ensureGeneratedPathsIgnored(root, projectFiles(root)), ['src/app/(templates)/'], JSON.stringify(line))
...
test('once a copy of src/app/ is written, only a line that ignores its directory counts, whatever the copy holds', async () => {
  ...
  const cases: { name: string; gitignore: string; added: string[] }[] = [
    {
      ...
      gitignore: 'src/app/(templates)/\n!src/app/(templates)/\n.nextspark/backups/\n!.nextspark/backups/\n' +
        '.nextspark/sync-state.json\n!/.nextspark/sync-state.json\napp.backup.*/\n!app.backup.v*/\n',
      added: ['src/app/(templates)/', '.nextspark/registries/', '.nextspark/backups/', '.nextspark/sync-state.json', 'app.backup.v*/'],
    },
    {
...
test('without git to ask, a negation takes back only what it matches, as git reads it once the project is a repository', async () => {
  ...
  const lines = 'src/app/(templates)/\n.nextspark/registries/\n.nextspark/backups/\n.nextspark/sync-state.json\napp.backup.v*/\n'
  const cases: { name: string; gitignore: string; added: string[] }[] = [
    { name: 'a negation of another file', gitignore: `${lines}!README.keep\n`, added: [] },
    { name: 'negations of files under the places, which git never looks into', gitignore: `${lines}!src/app/(templates)/keep.tsx\n!.nextspark/backups/**\n!app.backup.v*/**\n`, added: [] },
    { name: 'a negation of the templates directory by a glob', gitignore: `${lines}!src/app/(templ*)/\n`, added: ['src/app/(templates)/'] },
    { name: 'a negation of the sync state by its name', gitignore: `${lines}!sync-state.json\n`, added: ['.nextspark/sync-state.json'] },
    { name: 'a negation of some copies of src/app/', gitignore: `${lines}!app.backup.v1*/\n`, added: ['app.backup.v*/'] },
...
test('the rules git reads from outside the project count: its exclude file, core.excludesFile, the .gitignore files above the project, and core.ignorecase', async () => {
  ...
  try {
    ...
    await writeFile(join(top, 'rules'), 'src/app/(templates)/\n')
    await symlink(join(top, 'rules'), join(root, '.gitignore'))

    await writeFile(join(top, '.git/info/exclude'), 'apps/web/src/app/(templates)/\n')
    ...
    assert.deepEqual(planGitignore(root), {
      add: [],
      leftForGit: [{ entry: 'src/app/(templates)/', why: "src/app/.gitignore line 1 (!(TEMPLATES)/) takes it back, and git reads that over a line the project's .gitignore adds" }],
    })
    await mkdir(join(root, 'src/app/(templates)'))
    await writeFile(join(root, 'src/app/(templates)/page.tsx'), '')
    assert.equal(gitIgnores(root, 'src/app/(templates)/page.tsx'), false, 'git takes it back')
  } finally {
    await cleanup()
  }
})
