...
- Configure roles and entity permissions in \`config/permissions.config.ts\` with \`PERMISSIONS_CONFIG_OVERRIDES\`. Enforce those permissions at the server/API boundary, not only in hidden UI controls.
...
- Create exactly \`config.ts\`, \`schema.ts\`, \`fields.ts\`, \`component.tsx\`, and \`index.ts\` in \`blocks/{slug}/\`. The source tree is project-owned; do not edit generated registry files.
...
- Export the block config, schema, fields, and component through \`index.ts\`. Keep \`config.ts\` aligned with its slug, scope, and category, and keep component styles within the project conventions.
