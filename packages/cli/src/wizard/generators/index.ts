...
/**
 * Ensure a project has the root-first workspace template without discarding
 * package-manager policy already present in the destination.
 *
...
export async function mergeWorkspaceYaml(templatePath: string, destPath: string): Promise<void> {
  ...
  // Root-first source and local plugins are not independent workspace packages.
  const existing = await fs.readFile(destPath, 'utf-8')
  await fs.writeFile(destPath, setPackageEntries(existing, []))
}
...
export async function updatePackageJson(config: WizardConfig): Promise<void> {
  ...
  const scriptsToAdd: Record<string, string> = {
    ...
    'test': 'node node_modules/@nextsparkjs/core/scripts/test/jest.mjs',
    ...
    'allure:generate': 'allure generate tests/cypress/allure-results --clean -o tests/cypress/allure-report',
    'allure:open': 'allure open tests/cypress/allure-report',
  }
...
export async function generateProject(
  ...
  try {
    ...
    await fs.writeFile(
      path.join(process.cwd(), 'nextspark.config.ts'),
      `import { defineConfig } from '@nextsparkjs/core/lib/config'\n\nexport default defineConfig({\n  plugins: ${JSON.stringify(plugins)},\n  template: { name: ${JSON.stringify(projectTemplate)}, version: ${JSON.stringify(getNextSparkVersion())} },\n})\n`,
      'utf-8'
    )
