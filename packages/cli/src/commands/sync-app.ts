...
export async function syncAppCommand(options: SyncAppOptions): Promise<void> {
  ...
  try {
    ...
    // Get all template files (excluding (templates)/ auto-generated folder)
    ...
    if (customFiles.length > 0 && options.verbose) {
      ...
      if (customFiles.length > 10) {
        console.log(chalk.gray(`    ... and ${customFiles.length - 10} more`));
      }
    }
    ...
    if (options.backup && !options.dryRun) {
      const backupDir = join(projectRoot, `app.backup.${Date.now()}`);
    ...
    if (customFiles.length > 0) {
      ...
      if (customFiles.length > 5) {
        console.log(chalk.gray(`    ... and ${customFiles.length - 5} more\n`));
      } else {
        console.log();
      }
    }
