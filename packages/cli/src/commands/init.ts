...
`);
  ...
  // Icon Registry
  // Core imports this one unconditionally (lib/icons.ts), so leaving it out
  // does not degrade icons — it fails to resolve the module. Empty until a real
  // registry build fills it in; every name falls back until then.
  writeFileSync(join(registriesDir, 'icon-registry.ts'), `// Auto-generated by nextspark init
