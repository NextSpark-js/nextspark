...
import type { TestIdPattern, CypressIdPattern } from '../types/testing.types'
 ...
 */
export function createPriorityAttr(
...
export const testingPatterns = {
  ...
  task: {
    card: (taskId: string) => createTestingProps({
  ...
  // Form components
  form: {
    input: (fieldName: string) => createTestingProps({
 ...
 * import { createEntityTestingHelper } from './testing-utils'
...
export function createEntityTestingHelper(entitySlug: string): EntityTestingHelper {
  return {
    ...
    get: (component: string, detail?: string) => createEntityCyId(entitySlug, component, detail),
    ...
    fieldOption: (name: string, value: string) => createEntityCyId(entitySlug, `field-${name}-option`, value),
    ...
    filterTrigger: (field: string) => createEntityCyId(entitySlug, `filter-${field}`, 'trigger'),
    filterOption: (field: string, value: string) => createEntityCyId(entitySlug, `filter-${field}-option`, value),

    // Actions
    action: (action: string, id: string) => createEntityCyId(entitySlug, `action-${action}`, id),
