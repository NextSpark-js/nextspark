 ...
 * ```
 */
export const signupContextStorage = new AsyncLocalStorage<SignupContext>()

/**
 * Helper to get current signup context
 * Returns undefined if not within a signupContextStorage.run() call
 */
export function getSignupContext(): SignupContext | undefined {
  return signupContextStorage.getStore()
}

/**
 * Helper to check if team creation should be skipped
 */
export function shouldSkipTeamCreation(): boolean {
  const context = getSignupContext()
  return context?.skipTeamCreation === true
}

/**
 * Helper to run code within a signup context
 */
export async function withSignupContext<T>(
  context: SignupContext,
  fn: () => Promise<T>
): Promise<T> {
  return signupContextStorage.run(context, fn)
}
