...
The **Route Handlers Registry** (`.nextspark/registries/route-handlers.ts`) is an auto-generated registry that provides **zero-runtime-I/O access** to all API route handlers from themes and plugins.
...
**Location:** `.nextspark/registries/route-handlers.ts`
**Auto-regenerates:** When route files change in `api/` or `plugins/*/api/`
...
async function loadPluginRoute(pluginName: string, routePath: string) {
  const routeFile = await import(`@/plugins/${pluginName}/api/${routePath}/route`)
  return routeFile
}
...
import * as plugin_ai_ai_history__id_ from '@/plugins/ai/api/ai-history/[id]/route'
...
// Dynamic route: plugins/ai/api/ai-history/[id]/route.ts
...
```typescript
// File: plugins/ai/api/ai-history/[id]/route.ts
...
```typescript
// File: plugins/storage/api/files/[...path]/route.ts
export async function GET(
  request: NextRequest,
...
```typescript
// File: api/analytics/[[...filters]]/route.ts
export async function GET(
  request: NextRequest,
...
# or: cd apps/dev && node ../../packages/core/scripts/build/registry.mjs
```
...
**File:** `plugins/tasks/api/tasks/[id]/route.ts`
...
import { getPluginRouteKeys, getThemeRouteKeys } from '@nextsparkjs/registries/route-handlers'
...
```typescript
// plugins/notifications/api/send/route.ts
import { NextRequest, NextResponse } from 'next/server'
import { authenticateRequest } from '@/core/lib/api/auth/dual-auth'
...
ls plugins/ai/api/generate/route.ts

# 2. Check route exports HTTP methods
cat plugins/ai/api/generate/route.ts | grep "export async function"

# 3. Rebuild registry
cd apps/dev && node ../../packages/core/scripts/build/registry.mjs

# 4. Verify route in generated registry
cat .nextspark/registries/route-handlers.ts | grep "ai/generate"
```
...
cd apps/dev && node ../../packages/core/scripts/build/registry.mjs
grep "ai/generate" .nextspark/registries/route-handlers.ts
```
...
cat .nextspark/registries/route-handlers.ts

# Search for specific route
grep "your-route-key" .nextspark/registries/route-handlers.ts
```
...
import { POST } from '@/plugins/ai/api/generate/route'
import { NextRequest } from 'next/server'
...
**Source:** `.nextspark/registries/route-handlers.ts` (auto-generated)
**Build Script:** `packages/core/scripts/build/registry.mjs` (lines 2566-2689)
