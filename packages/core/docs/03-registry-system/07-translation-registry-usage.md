...
The **Translation Registry** (`.nextspark/registries/translation-registry.ts`) is an auto-generated registry that provides **lazy-loading translation functions** for optimal i18n performance.
...
**Location:** `.nextspark/registries/translation-registry.ts`
**Auto-regenerates:** When translation files change in `messages/`
...
export default getRequestConfig(async ({ locale }) => {
  ...
  const messages = await loadThemeTranslation(projectTheme, locale)
...
const projectTheme = 'project'
const locales = getThemeLocales(projectTheme)
...
### Pattern 1: Load Translations for project

```typescript
// lib/i18n/load-translations.ts
import { loadThemeTranslation } from '@nextsparkjs/registries/translation-registry'

export async function loadProjectTranslations(locale: string) {
  const projectTheme = 'project'
...
import { loadThemeTranslation, hasThemeTranslation } from '@nextsparkjs/registries/translation-registry'
...
**Build Script:** `packages/core/scripts/build/registry.mjs` (lines 2697-2833)
