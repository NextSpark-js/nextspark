...
**Location:** `.nextspark/registries/permissions-registry.ts`
...
1. Reads `permissions.config.ts` from project
...
2. **Always regenerate after config changes** - Run `cd apps/dev && node ../../packages/core/scripts/build/registry.mjs`
