...
- **pnpm 9** - Fast, disk space efficient package manager with workspaces
...
- **Theme Registry** - project configuration and route handlers
...
cd apps/dev && node ../../packages/core/scripts/build/registry.mjs --watch  # Watch mode during development
```
...
**Location:** `.nextspark/registries/` (auto-generated - never edit manually)
...
**Pluggable themes with CSS imports and asset management**
...
**Build Process:**
```bash
# Next.js compiles the imported theme CSS as part of the app build
pnpm build
```
...
```typescript
// Generated by packages/core/scripts/build/registry.mjs
import { TRANSLATION_REGISTRY } from '@nextsparkjs/registries/translation-registry'
...
### Framework and project-source separation

The application architecture separates installed framework code from project-owned root-first source:

**Framework (`@nextsparkjs/core`)**
...
- One project presentation and configuration
- Local plugins under `plugins/`
- Dynamic entities under `entities/`
- Owned by the project and preserved across framework updates

**Benefits:**
- **Modularity**: Clear separation of concerns
- **Extensibility**: Add features without modifying core
- **Maintainability**: Core updates don't break content
- **Scalability**: Install-once templates provide distinct starting points
- **Testability**: Test framework and project source independently
