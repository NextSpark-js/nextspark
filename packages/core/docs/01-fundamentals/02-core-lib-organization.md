...
These files are generated into the consuming project's `.nextspark/registries/` (in this monorepo, `apps/dev/.nextspark/registries/`) and imported as `@nextsparkjs/registries/<name>`; core's `lib/` has no `registries/` directory.
...
cd apps/dev && node ../../packages/core/scripts/build/registry.mjs --watch  # Watch mode (auto-rebuild on changes)
```
...
2. ALWAYS regenerate with `cd apps/dev && node ../../packages/core/scripts/build/registry.mjs`
