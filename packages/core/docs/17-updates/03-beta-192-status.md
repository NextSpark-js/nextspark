...
A post-commit tarball clean-install and beta.191→beta.192 upgrade validation run found three delivery/build blockers: a `proxy.ts` template TypeScript strict-null error that failed `next build`'s type-check for every fresh project, `instrumentation.ts` never being copied into a generated project (so the startup check above never ran), and local-tarball dependency discovery missing `@nextsparkjs/testing`, a runtime dependency of `@nextsparkjs/core`, which broke every tarball-based install and upgrade until that version is published. The first two are fixed. The third is fixed for fresh projects only. A re-run against the fixes showed:

- A fresh project created from the tarballs installs, migrates, builds and starts with no manual steps, and receives `instrumentation.ts`.
- A beta.191 host upgraded to the tarballs builds and starts after `pnpm exec nextspark sync:app` and the documented route guards. Upgrading from local tarballs still needs a hand-added `pnpm.overrides` entry for `@nextsparkjs/testing`.

An upgrade from the registry after publication is expected not to need that override. This has not been verified, and that check remains pending. This status is not a release certification.
