...
export function generatePluginRegistry(plugins, config) {
  ...
  const pagesServerImports = plugins
    ...
    .map(plugin =>
      `import { devtoolsPage as ${toSafeIdentifier(plugin.name)}DevtoolsPage, superadminPage as ${toSafeIdentifier(plugin.name)}SuperadminPage } from '${plugin.importBase}/plugin.pages.server'`
    )
    .join('\n')
