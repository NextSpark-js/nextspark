 ...
 *
 * Generates block-registry.ts, block-registry.client.ts and block-registry.lazy.ts
 *
...
export { BLOCK_COMPONENTS } from './block-registry.lazy'
...
export * from './block-registry.client'
// Lazy components live in their own module, which a client component can import
// without the static imports below.
export { BLOCK_COMPONENTS } from './block-registry.lazy'
...
/**
 * Generate the lazy half: every block component behind React.lazy, and no
 * static import.
 *
 * Client components that render blocks (the editor canvas, pattern previews,
 * devtools) read this, so each block stays in a chunk of its own and loads
 * when it is shown. Reaching the components through block-registry.ts instead
 * carries its static SSR imports along, and with them every block and all a
 * block imports, into that client component's bundle.
 *
 * @param {Array} blocks - Discovered blocks
 * @returns {string} Generated TypeScript content
 */
export function generateBlockRegistryLazy(blocks) {
  const entries = blocks.map(block =>
    `  '${block.slug}': React.lazy(() => import('${block.paths.component}').then(mod => ({ default: Object.values(mod)[0] as React.ComponentType<any> })))`
  ).join(',\n')

  return `/**
 * Auto-generated Block Registry (lazy components)
 *
 * Generated at: ${new Date().toISOString()}
 * Blocks discovered: ${blocks.length}
 *
 * Block components behind React.lazy and nothing imported statically, so a
 * client component can render blocks while each one stays in its own chunk.
 *
 * DO NOT EDIT - This file is auto-generated by scripts/build-registry.mjs
 */
