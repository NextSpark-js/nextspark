...
/**
 * The comment a copy of an app/ layout in app/(templates)/ starts with. A
 ...
 * project: the ones a route generated into app/(templates)/ needs copied beside
...
export async function generateMissingPages(templates, config = null, analysis = null) {
  ...
  if (created > 0 || updated > 0 || removed > 0) {
    log(`app/(templates): ${created} new, ${updated} updated, ${removed} removed`, 'success')
  } else {
...
/** Where a file under app/ is generated inside app/(templates)/. */
function templatesTreePath(appPath) {
  return join(rootDir, 'app', '(templates)', appPath.replace(/^app\//, ''))
}

/**
 * Everything the registry build puts in app/(templates), as absolute path to
