/**
 * Tests for how the registry build keeps src/app/(templates): the tree holds exactly
...
test('a stale copy of an app layout is refreshed, and what it held is backed up', async () => {
  ...
  try {
    await writeProjectFile(root, 'src/app/dashboard/layout.tsx', DASHBOARD_LAYOUT)
    await writeProjectFile(root, 'src/app/(templates)/dashboard/layout.tsx', "'use client'\n// copied by an older release\n")
    const page = await writeTemplate(root, 'dashboard/reports/page.tsx', 'page', PAGE)

    await generateMissingPages([page], { projectRoot: root })

    const copy = await readFile(join(root, 'src/app/(templates)/dashboard/layout.tsx'), 'utf8')
    ...
    assert.deepEqual(await backedUpFiles(root), [
      { path: 'src/app/(templates)/dashboard/layout.tsx', content: "'use client'\n// copied by an older release\n" }
    ])
  } finally {
...
test('a layout copy no generated route needs any more is removed, and backed up', async () => {
  const root = await createProject()
  try {
    await writeProjectFile(root, 'src/app/(templates)/old/layout.tsx', DASHBOARD_LAYOUT)
    const page = await writeTemplate(root, 'pricing/page.tsx', 'page', PAGE)

    await generateMissingPages([page], { projectRoot: root })

    assert.equal(existsSync(join(root, 'src/app/(templates)/old/layout.tsx')), false)
    assert.equal(existsSync(join(root, 'src/app/(templates)/old')), false)
    assert.deepEqual(await backedUpFiles(root), [{ path: 'src/app/(templates)/old/layout.tsx', content: DASHBOARD_LAYOUT }])
  } finally {
    await rm(root, { recursive: true, force: true })
  }
})
...
test('the route file of a template whose app route now exists is removed, and backed up', async () => {
  ...
  try {
    const page = await writeTemplate(root, 'pricing/page.tsx', 'page', PAGE)
    await generateMissingPages([page], { projectRoot: root })
    const generated = await readFile(join(root, 'src/app/(templates)/pricing/page.tsx'), 'utf8')

    await writeProjectFile(root, 'src/app/pricing/page.tsx', PAGE)
    await generateMissingPages([page], { projectRoot: root })

    assert.equal(existsSync(join(root, 'src/app/(templates)/pricing/page.tsx')), false)
    assert.deepEqual(await backedUpFiles(root), [{ path: 'src/app/(templates)/pricing/page.tsx', content: generated }])
  } finally {
    await rm(root, { recursive: true, force: true })
  }
})
...
test('regenerating a tree that is already up to date writes nothing and backs nothing up', async () => {
  ...
  try {
    ...
    const generatedFiles = ['dashboard/layout.tsx', 'dashboard/reports/page.tsx', 'dashboard/reports/layout.tsx']
      .map(file => join(root, 'src/app/(templates)', file))
...
test('two builds in the same millisecond keep both backups', async () => {
  ...
  try {
    ...
    await writeProjectFile(root, 'src/app/(templates)/dashboard/layout.tsx', '// first hand edit\n')
    await generateMissingPages([page], { projectRoot: root })
    await writeProjectFile(root, 'src/app/(templates)/dashboard/layout.tsx', '// second hand edit\n')
    await generateMissingPages([page], { projectRoot: root })
...
test('a dotfile left in src/app/(templates) is removed like any other file, and backed up', async () => {
  const root = await createProject()
  try {
    await writeProjectFile(root, 'src/app/(templates)/dashboard/.user-note', 'remember to check the layout\n')
    const page = await writeTemplate(root, 'pricing/page.tsx', 'page', PAGE)

    await generateMissingPages([page], { projectRoot: root })

    assert.equal(existsSync(join(root, 'src/app/(templates)/dashboard/.user-note')), false)
    assert.deepEqual(await backedUpFiles(root), [
      { path: 'src/app/(templates)/dashboard/.user-note', content: 'remember to check the layout\n' }
    ])
  } finally {
    await rm(root, { recursive: true, force: true })
  }
...
test('a backup the build takes is out of git from the moment it is written, whatever the project\'s .gitignore leaves in', { skip: process.getuid?.() === 0 }, async () => {
  ...
  try {
    ...
    await writeProjectFile(root, '.gitignore', 'src/app/(templates)/\n!.nextspark/backups/**\n')
    await writeProjectFile(root, 'src/app/dashboard/layout.tsx', DASHBOARD_LAYOUT)
    await writeProjectFile(root, 'src/app/(templates)/dashboard/layout.tsx', '// replaced first\n')
    await writeProjectFile(root, 'src/app/(templates)/old/page.tsx', '// removed after\n')
    const page = await writeTemplate(root, 'dashboard/reports/page.tsx', 'page', PAGE)

    // The page it removes can't be read, so the build stops once it has backed up the layout it replaces
    await chmod(join(root, 'src/app/(templates)/old/page.tsx'), 0o000)
    await assert.rejects(generateMissingPages([page], { projectRoot: root }), /EACCES/)
    assert.deepEqual((await backedUpFiles(root)).map(({ path }) => path), ['src/app/(templates)/dashboard/layout.tsx'])
    assert.deepEqual(nextsparkPathsForGit(root), [], 'git picks up nothing while the build is partway')

    await chmod(join(root, 'src/app/(templates)/old/page.tsx'), 0o644)
    await generateMissingPages([page], { projectRoot: root })
...
test('a file where the tree needs a directory, or a directory where it needs a file, is planned, backed up and removed before the build writes there', async () => {
  ...
  try {
    await writeProjectFile(root, 'src/app/dashboard/layout.tsx', DASHBOARD_LAYOUT)
    await writeProjectFile(root, 'src/app/(templates)/dashboard', '// a file where a directory goes\n')
    await writeProjectFile(root, 'src/app/(templates)/pricing/page.tsx/note.md', 'a directory where a file goes\n')
    await writeProjectFile(root, 'src/app/(templates)/pricing/page.tsx/.DS_Store', '')
    await mkdir(join(root, 'src/app/(templates)/pricing/page.tsx/empty'), { recursive: true })
    ...
    assert.deepEqual(await planMissingPages([reports, pricing], { projectRoot: root }), {
      create: ['src/app/(templates)/dashboard/layout.tsx', 'src/app/(templates)/dashboard/reports/page.tsx', 'src/app/(templates)/pricing/page.tsx'],
      replace: [],
      remove: ['src/app/(templates)/dashboard', 'src/app/(templates)/pricing/page.tsx/note.md'],
    })
    ...
    for (const file of ['dashboard/layout.tsx', 'dashboard/reports/page.tsx', 'pricing/page.tsx']) {
      assert.ok((await stat(join(root, 'src/app/(templates)', file))).isFile(), `${file} is written`)
    }
    assert.deepEqual((await backedUpFiles(root)).sort((a, b) => a.path.localeCompare(b.path)), [
      { path: 'src/app/(templates)/dashboard', content: '// a file where a directory goes\n' },
      { path: 'src/app/(templates)/pricing/page.tsx/note.md', content: 'a directory where a file goes\n' },
    ])
  } finally {
    await rm(root, { recursive: true, force: true })
  }
...
test('a line the build prints that names a path whose name would break or reorder it is escaped, once the console is guarded as the build guards it', async () => {
  ...
  try {
    for (const [name] of names) await writeProjectFile(root, `src/app/(templates)/old/${name}`, '// mine\n')
    const page = await writeTemplate(root, 'pricing/page.tsx', 'page', PAGE)
    ...
    for (const [, shown] of names) {
      assert.ok(lines.some(line => line.startsWith(`"⚠️ src/app/(templates): backed up src/app/(templates)/old/${shown} to .nextspark/backups/`)), `${shown} is named escaped`)
    }
  } finally {
    await rm(root, { recursive: true, force: true })
  }
