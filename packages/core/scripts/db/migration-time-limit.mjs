...
/** The pg client options that carry the limit, or none without one. */
export function clientTimeLimits(limit) {
  if (!limit) return {};
  return { statement_timeout: limit.statementMs, query_timeout: limit.queryMs };
}
...
async function endSession(connectionString, processID, limit) {
  ...
  const admin = new Client({
    ...
    connectionTimeoutMillis: waitMs,
