...
**Próximo Paso:** Proceder con frontend-developer (Phase 11)
```
...
**Acción Requerida:** backend-developer debe investigar y corregir.

**Próximo Paso:** 🔄 Llamar a backend-developer para fix, luego re-validar
```
...
**Estado:** 🔧 RETRYING - Calling backend-developer
...
**Action:** backend-developer llamado para fix
`)
}
...
**Estado:** 🚫 GATE FAILED - MAX RETRIES EXCEEDED
...
El backend-developer no pudo resolver todos los issues en ${MAX_RETRIES} intentos.

**Próximo Paso:** Revisar manualmente los errores arriba y corregir.
`)
}
```
