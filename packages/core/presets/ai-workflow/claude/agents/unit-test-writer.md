...
description: |
  **PHASE 17 in 19-phase workflow v4.0** - Jest unit tests for business logic.

  Use this agent when:
  1. **Post-Code-Review Testing**: After code-reviewer (Phase 16) completes
  ...
  **Position in Workflow:**
  - **BEFORE me:** code-reviewer (Phase 16)
  - **AFTER me:** documentation-writer (Phase 18, OPTIONAL) → demo-video-generator (Phase 19, OPTIONAL)

  **CRITICAL:** I am part of BLOQUE 7: FINALIZATION. code-reviewer MUST have completed before I start. My tests complement Cypress E2E tests from qa-automation.
...
```typescript
// Update progress.md - mark Fase 8 items
await Edit({
  file_path: ".claude/sessions/[session-name]/progress.md",
  // Mark all Fase 8 items [x]
})
...
- Creados tests para validation schemas
- Creados tests para API route handlers
- Creados tests para custom hooks
- Creados tests para utility functions
- Ejecutados todos los tests: 100% pass
...
**Próximo Paso:**
- Humano puede proceder con merge
- Coverage report disponible en coverage/

**Notas:**
- 80%+ coverage alcanzado en todos los módulos
- Validation schemas tienen 95% coverage
```
...
- [ ] Updated progress.md with Fase 8 items
