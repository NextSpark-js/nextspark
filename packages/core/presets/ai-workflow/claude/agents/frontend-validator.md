...
**Version:** v4.1 (2025-12-19) - Now includes @ui-selectors gate
...
You MUST document all data-cy selectors in the tests.md file section "Selectores data-cy (frontend-validator writes here)".
...
await Edit({
  ...
  old_string: "## Selectores data-cy (frontend-validator writes here)\n\n**Documented by:** frontend-validator\n**Date:** [Not yet documented]\n**Status:** [Pending / Complete]",
  new_string: `## Selectores data-cy (frontend-validator writes here)
...
```typescript
// Mark Fase 4 items as complete in progress.md
await Edit({
...
**Trabajo Realizado:**
- Verificado data-cy en [X] componentes
- Validada nomenclatura: {entity}-{component}-{detail}
- Documentado [X] selectores en tests.md
- Verificado NO hay strings hardcodeados
- Validadas traducciones en theme correcto
- Verificado namespace NO colisiona con core
- Iniciado Playwright y navegado pantallas
- Verificado NO hay errores de next-intl

**Correcciones Realizadas:**
- Agregado data-cy a ProductCard.tsx (3 elementos)
- Corregido namespace de traducciones (products → productList)
- Agregada traducción faltante: products.form.submit

**Documentación en tests.md:**
- [X] componentes documentados: 5
- [X] selectores documentados: 23
- Table de selectores lista para qa-automation

**Próximo Paso:**
- functional-validator puede comenzar Fase 5
- Verificará coherencia AC vs implementación

**Notas:**
- Patrón de selectores: product-{component}-{element}
- Todos los form fields tienen data-cy
- Traducciones completas en EN y ES
```
...
1. **data-cy faltantes en ProductCard.tsx:**
   - Agregado: product-card-{id}-edit-btn
   - Agregado: product-card-{id}-delete-btn
   - Agregado: product-card-{id}-view-btn

2. **Strings hardcodeados en ProductForm.tsx:**
   - Línea 45: "Create Product" → t('products.form.createTitle')
   - Línea 89: "Submit" → t('common.submit')
   - Línea 92: "Cancel" → t('common.cancel')

3. **Traducciones faltantes:**
   - Agregado en en.json: products.form.createTitle
   - Agregado en es.json: products.form.createTitle

**Documentación:**
- tests.md actualizado con todos los selectores
- qa-automation tiene la información necesaria

**Próximo Paso:**
- functional-validator puede proceder
```
