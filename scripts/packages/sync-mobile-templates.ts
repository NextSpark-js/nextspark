#!/usr/bin/env npx tsx
/**
 * sync-mobile-templates.ts - Sync apps/mobile/app with packages/mobile/templates/app
 *
 * Source of Truth:
 *   apps/mobile/app/           <- SOURCE (development)
 *   packages/mobile/templates/app/ <- DISTRIBUTION (packaged)
 *
 * Usage:
 *   pnpm sync:mobile-templates          # Interactive mode - shows diff and asks to sync
 *   pnpm sync:mobile-templates --sync   # Copy from apps/mobile → templates
 *   pnpm sync:mobile-templates --check  # Check only (for CI) - exits 1 if out of sync
 *
 * Exit codes:
...
// Paths
const SOURCE_DIR = join(REPO_ROOT, 'apps/mobile/app')
const TARGET_DIR = join(REPO_ROOT, 'packages/mobile/templates/app')
...
async function main() {
  ...
  console.log()
  console.log(`${CYAN}========================================${NC}`)
  console.log(`${CYAN}  NextSpark - Mobile Template Sync${NC}`)
  console.log(`${CYAN}========================================${NC}`)
  console.log()

  console.log(`${DIM}Source:${NC} apps/mobile/app/`)
  console.log(`${DIM}Target:${NC} packages/mobile/templates/app/`)
  console.log()
  ...
  if (diffs.length === 0) {
    console.log(`${GREEN}✓ Mobile templates are in sync${NC}`)
    console.log()
    process.exit(0)
  }
  ...
  // Handle modes
  if (checkMode) {
    console.log(`${RED}✗ Mobile templates are out of sync${NC}`)
    console.log(`Run ${CYAN}pnpm sync:mobile-templates --sync${NC} to update templates.`)
    process.exit(1)
  }
  ...
  console.log(`${YELLOW}Mobile templates are out of sync.${NC}`)
  console.log(`Run ${CYAN}pnpm sync:mobile-templates --sync${NC} to copy changes to templates.`)
  console.log(`Run ${CYAN}pnpm sync:mobile-templates --check${NC} for CI validation.`)
  console.log()
  process.exit(0)
}
