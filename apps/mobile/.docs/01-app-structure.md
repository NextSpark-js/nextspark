...
# address, as Metro reports it - a LAN address in Metro's default mode, which
# is what the Android emulator gets too. The emulator only gets 10.0.2.2
# instead when Metro's own address is loopback (Expo started with
# `--localhost`, or an `adb reverse tcp:8081 tcp:8081` tunnel), since it is a
# separate machine from a loopback address's point of view. Only set this to
# override the detection, e.g. a backend that isn't the local dev server:
