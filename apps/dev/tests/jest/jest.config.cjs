/**
 * Jest Configuration for the root-first development project
 *
 * This config works in both monorepo and npm mode.
 * Run with: pnpm exec jest --config tests/jest/jest.config.cjs
 */
...
const monorepoCore = path.resolve(projectRoot, '../../packages/core')
const isNpmMode = !fs.existsSync(monorepoCore)
